...
**A single retrieval mechanism over both — which is what the vector-database option implies — re-indexes a dimension table for the volatile half and adds retrieval risk to a corpus that already fits.** Retrieval over fifty items retrieves the wrong one sometimes; sending all fifty never does.

## The dictionary lives in the SQLX, because a prompt is not a source of truth

A prompt is a deploy artefact: editable in a file nobody who owns a metric reviews. So the definitions live where the metric is implemented, in the Dataform `config` block, changed in the same pull request as the SQL they describe.
...
**The dictionary's job is to make the model's reading explicit, not to guarantee it guessed right.**

## `resolve_entity` is a query, not an index

`publisher_id` and `ad_unit_id` are already human-readable — the names the Yield team says out loud *are* the values in the column — so the lookup needs no mapping table, only a fuzzy match against the live dimension values in the semantic views:
...
- **`max_distance => 4` with a filter of `< 4`, not `<= 4`.** `EDIT_DISTANCE` returns `max_distance` when the true distance exceeds it — that is what lets it stop early — so a predicate at the cap matches every name in the table. The cap is the performance knob; the filter sits one below it.
...
**The tool returns ranked candidates, not an answer.** Where more than one scores close, the copilot asks which. Silently picking the nearest is the worst failure available to it: a confident, well-formed answer about the wrong client, indistinguishable from a right one. One question costs a turn.
