...
**Not deferred to Gold either, though the join keys allow it.** `ref_fx_rate` and `ref_revenue_share` are keyed at or above Gold's grain (`day`, `currency`, `publisher_id`), so `SUM(price) × rate` inside the Gold build would be arithmetically identical — Gold already scans full Silver row detail to compute `auctions_with_bid` below, so the join would cost no extra scan. What it would cost is detection speed: the guardrail below is a Dataform assertion, and moving the join to Gold moves the assertion from Silver's ~30-minute cadence to Gold's ~4-hour one. An expired contract row is silent-wrong-money, not a missing row — the failure this design is least willing to detect late. A rarer, human-triggered repair (a rate correction, rerun as a runbook) is the cheaper thing to keep expensive.
...
| **Enrichment deferred to the Gold build** | Arithmetically equivalent — the join keys sit at or above Gold's grain — but it moves the null-`publisher_payout` assertion from Silver's ~30-min cadence to Gold's ~4h one, on the one failure mode that is silent-wrong-money |
