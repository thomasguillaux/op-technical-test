...
| A producer re-stamping `auction_timestamp` | monitor | trailing 3 days: any `event_id` with more than one row, or any row where `DATE(auction_timestamp) != auction_day` | A retry that moves the auction clock across midnight. The `MERGE` looks only in the days its own batch carries, so it either inserts a second row — Gold counts the event twice — or updates in place and leaves the row in the wrong partition. Neither errors |
