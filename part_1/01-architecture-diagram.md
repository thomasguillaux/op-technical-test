...
|---|---|---|---|
| 1 | Collector → `events` topic | Publish. The producer splits the envelope: `event_id`, `source_id`, `publisher_id`, `ssp_id`, `event_type` as named fields, plus the auction context — including `auction_timestamp` — stamped on every event. Everything else under `payload` | seconds |
...
| 3 | topic → GCS raw archive | **Cloud Storage export subscription.** Standard class, 7 days. A second subscription, not a copy of Bronze: this copy never passes through BigQuery | seconds |
| 4 | topic → dead-letter topic | A message that fails the topic schema is dead-lettered **alone**; the stream continues. The quality job checks queue depth | seconds |
...
| 6 | Silver → Gold | **Dataform.** Rebuild the days whose Silver rows changed, within a trailing 3-day window. Rows are at hourly grain | hourly |
| 7 | Gold → semantic views | BigQuery views. Each metric formula written once, in one place. Daily is a view over the hourly table | query time |
...
**1. Two writes from one topic, not one write and a copy.** Bronze and the archive are siblings: the topic feeds both directly. A scheduled export from Bronze to GCS would save the second export fee, but it would place the safety copy *downstream* of the system it protects us from. A corrupting write into Bronze would be faithfully copied into the archive.
...
Two arrows out of the topic means two failure domains. One arrow through Bronze means one. **That independence is now the archive's *only* justification:** past day 7 there is no rebuild path at all, so a copy that has never been inside BigQuery is the only thing that survives a mistake made inside BigQuery.
...
| **An elapsed hour with zero events** | count = 0 for a closed hour | At 23,000/second an empty hour is unambiguously a failure, never a lull. No baseline, no threshold to argue about |
