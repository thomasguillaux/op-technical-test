...
**The instinct is that raw is the irreplaceable copy and must be kept longest. Under this rule it is the reverse:** raw is transient, so the irreplaceable copy is the first layer *allowed* to persist. Silver is the source of truth, anonymous and retained indefinitely; Bronze is a landing and replay buffer whose window we do not control.
